...
- [x] EVT-001: Implement API Gateway HTTP command routes with authentication integration and request/correlation ID propagation. Infra owns the edge route, authorizer wiring, and request/correlation ID propagation plumbing; command semantics remain owned by the service repos and shared contracts. Current cost-reduced state: HTTP command routes synthesize with JWT authorizer wiring, one VPC link, request/correlation ID header propagation, and one internal ALB listener to the shared dogfood ECS/Fargate runtime while route metadata preserves owning services. `GET /health` is the only intentionally placeholder-backed route.
- [x] EVT-003: Implement SSE-capable service or Lambda path and edge route for session event streaming. Infra owns the deployable streaming path and edge route; session event payload contracts and publishing semantics remain owned by contracts, session-events, and orchestration repos. Current cost-reduced state: the browser SSE route is hosted by the shared public ALB HTTPS listener to the dogfood Fargate runtime with 900 second idle timeout, heartbeat/replay env vars, health checks, logs, and IAM boundaries. Lambda is not used for primary SSE.
...
- [x] Let CDK create static web app hosting and DNS from `WebAppBaseUrl` for repeatable dev/gamma/prod deploys. Current behavior creates a private S3 assets bucket, CloudFront distribution, companion `us-east-1` ACM certificate stack, Route 53 alias, and deployment outputs; building/uploading `ai-assist-web/dist` remains an explicit deployment handoff.
- [x] M10-T2: Document that `https://dev.melsifen-ai-assist.com` is supporting infrastructure for OAuth redirects, hosted assets, diagnostics, or fallback setup while the Google Docs side-panel/browser-extension shell is the primary dogfood UI.
...
- [x] M10-T3: wire the shared dogfood runtime so `/resources` and context preview derive tenant/user identity from the product session, use auth-service Google OAuth token handoff, and call the Google Docs adapter read/list dependencies in-process. Deployed live proof still requires redeploy plus approved Google OAuth and controlled document configuration.
