...
- [x] EVT-001: Implement API Gateway HTTP command routes with authentication integration and request/correlation ID propagation. Infra owns the edge route, authorizer wiring, and request/correlation ID propagation plumbing; command semantics remain owned by the service repos and shared contracts. Current cost-reduced state: HTTP command routes synthesize with JWT authorizer wiring, one VPC link, request/correlation ID header propagation, and one shared ALB listener to the dogfood ECS/Fargate runtime while route metadata preserves owning services. `GET /health` is the only intentionally placeholder-backed route.
